...
const mongooseBread: MongooseBreadPlugin = function (schema: Schema, pluginOptions: Partial<MongooseBreadOptions>) {
	...
	if (searchableFields && searchableFields.length) {
		_pluginOptions.searchableFields = checkSchema(schema).getSearchableFieldsOfTypeString(searchableFields);
	}
	...
	if (_pluginOptions.softDelete) {
		...
		if (softDeleteOptions.indexFields) {
			...
			const deleteByIsValid = deletedBy && requestUserIdPath && typeof requestUserIdPath == "string";
