...
function softDeleteFactory(pluginOptions) {
  var deletedBy = pluginOptions.softDeleteOptions.deletedBy;
  ...
  return function softDelete(options) {
    ...
    var userId = options.userId,
      query = options.query;
    var mergeSoftDeleteAndDocs = function mergeSoftDeleteAndDocs(_docs) {
      return Promise.all([deletedBy && userId ? _this.delete(query, userId).orFail() : _this.delete(query).orFail(), Promise.resolve(_docs)]);
    };
    return this.findWithDeleted(query, {
      _id: 1
    }).then(mergeSoftDeleteAndDocs).then(toBreadResult).catch(toBreadError);
  };
}
