...
function addFactory(pluginOptions) {
  var _pluginOptions$custom = pluginOptions.customLabels,
    ...
    createdCount = _pluginOptions$custom.createdCount;
  ...
  return function add(options) {
    ...
    var mergeCreateAndCount = function mergeCreateAndCount(result) {
      var query = bulk ? {
        _id: {
          $in: _toConsumableArray(result.map(function (d) {
            return d._id;
          }))
        }
      } : {
      ...
      }), _this.find(query, projection).select(select).sort(sort).lean(lean).limit(limit).orFail().then(parseLean)]);
    };
    return this.create(payload).then(mergeCreateAndCount).then(toBreadResult).catch(toBreadError);
  };
}
