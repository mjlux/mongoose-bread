...
function rehabilitateFactory(pluginOptions) {
  ...
  return function rehabilitate(options) {
    ...
    var mergeRestoreAndDocs = function mergeRestoreAndDocs(_docs) {
      return Promise.all([_this.restore(query), Promise.resolve(_docs)]);
    };
    return this.findWithDeleted(query, {
      _id: 1
    }).then(mergeRestoreAndDocs).then(toBreadResult).catch(toBreadError);
  };
}
